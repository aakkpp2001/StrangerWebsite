...
export function drawWorldScene(ctx: CanvasRenderingContext2D, w: number, h: number, time: number) {
  // Alien sky — dusk purple to dark
  for (let y = 0; y < h; y++) {
    const t = y / h;
    const r = Math.floor(15 + t * 10);
    const g = Math.floor(12 + t * 5);
  ...
  // Stars
  stars(ctx, w, h, 50, 3.0, time, 0, h * 0.55);

  // Alien landscape — rolling hills with unusual colors
  ...
  // Small settlement — distant buildings
  const setX = w * 0.3;
