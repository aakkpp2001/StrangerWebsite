import { px, fillRow, circle, glowCircle, lerp, stars, smoothNoise } from '@/components/PixelArtCanvas';
...
export function drawHeroScene(ctx: CanvasRenderingContext2D, w: number, h: number, time: number) {
  ...
  // Mid-ground hills — layered, scanline fill
  ...
  // Ground reflection near character
  for (let i = 0; i < 30; i++) {
    const rx = charX - 15 + i;
    if (rx >= 0 && rx < w) {
